...
func main() {
	...
	var nestingDepth = 2 // задаем глубину вложенности при поиске нужных файлов, максимальная 4
	...
	go func() {
		...
		case syscall.SIGUSR1:
			// Обработать сигнал SIGUSR1
			log.Println("INPUT SIGUSR1: display current directory and current search depth")
		case syscall.SIGUSR2:
			log.Println("INPUT SIGUSR2: search depth will be increased (+2)") // Обработать сигнал SIGUSR2
		default:
			// Текстовая информация та, которая соответствует всем каналам кроме SIGUSR1 и SIGUSR2
